...
router.get("/files", async (req, res) => {
  ...
  const result = folderId
    ? await query.where(eq(filesTable.folderId, folderId)).orderBy(sql`${filesTable.createdAt} DESC`)
    : await query.where(isNull(filesTable.folderId)).orderBy(sql`${filesTable.createdAt} DESC`);
...
router.post("/files", async (req, res) => {
  ...
  const { originalName, size, mimeType, storagePath, shareToken, folderId } = parsed.data;
  const id = randomUUID();
  const [file] = await db
    .insert(filesTable)
    .values({ id, originalName, size, mimeType, storagePath, shareToken, folderId: folderId || null })
    .returning();
...
router.put("/files/:id/move", async (req, res) => {
  ...
  const [updated] = await db
    .update(filesTable)
    .set({ folderId: bodyParsed.data.folderId })
...
router.post("/folders", async (req, res) => {
  ...
  const [folder] = await db
    .insert(foldersTable)
    .values({ id, name: parsed.data.name, shareToken: createShareToken() })
    .returning();
