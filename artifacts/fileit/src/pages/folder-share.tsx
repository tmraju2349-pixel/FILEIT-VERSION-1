...
import { AlertCircle, Download, File as FileIcon, FileCode2, Folder, Loader2 } from "lucide-react";
...
export default function FolderSharePage() {
  ...
  const { data, isLoading, isError } = useGetFolderByShareToken(token, {
    query: {
      enabled: !!token,
      queryKey: getGetFolderByShareTokenQueryKey(token),
      retry: false,
    },
  });
  ...
  const handleDownload = () => {
    ...
    link.download = `${data.folder.name}.zip`;
    ...
    return (
      <div className="flex min-h-[100dvh] items-center justify-center bg-background p-4">
        <div className="w-full max-w-md space-y-4 rounded-2xl border border-border bg-card p-8 text-center">
          ...
          <p className="text-muted-foreground">This share link may be invalid or the folder was deleted.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-[100dvh] bg-background">
      <header className="border-b border-border bg-background/80 backdrop-blur-md">
        <div className="container mx-auto flex h-16 max-w-4xl items-center gap-3 px-4">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-bold tracking-tight">Fileit</p>
            <p className="truncate text-xs text-muted-foreground">Shared folder</p>
          </div>
